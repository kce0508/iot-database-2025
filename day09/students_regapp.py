...
def showDatas():
    ...
    ### DB 연결. 커넥션 객체 생성 -> 커서 -> 쿼리 실행 -> 커서로 데이터 패치 -> 커서종료 -> 커넥션 종료
...
def addData():
    ...
    showDatas()     # DB 테이블의 모든 데티어 조회해서 트리뷰에 표시
...
myFont = Font(family='NanumGothic', size=10)    # 이후에 화면 위젝에 지정할 동일 폰트 생성
