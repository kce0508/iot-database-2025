...
/* 문제3 - 월별로 입사안 사원수가 아래와 같이 행별로 출력되도록 하시오. 12행
*/
-- 형변환 함수 cast(), convert()
select cast('-09' as unsigned);	-- unsigned(양수만 숫자형)
select convert('90', signed); -- signed(음수포함숫자형)
...
/* 샘플 - 부서와 업무별 급여합계를 구하고 신년도 급여수준 레벨을 지정하려고 함.
			부서 번호와 업무를 기본으로 전해 행을 그룹별로 나누어 급여합계와 인원수를 출력(20개행)
*/
...
-- RANK
/* 샘플 - 분석함수 NITLE() 사용, 부서별 급여 합계를 구하시오. 급여가 제일 큰 것이 1, 제일 작은 것이 4가 되도록 등급을 나눔(12행)
*/
